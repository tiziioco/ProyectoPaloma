...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- PASO A: GUARDAR EL CUIDADOR SELECCIONADO (BLINDADO) ---
    if (isset($_POST['id_superior']) && !empty($_POST['id_superior'])) {
        $id_cuidador = intval($_POST['id_superior']);
        
        // Verificamos de forma estricta si el cuidador existe físicamente en MySQL
        $check_cuidador_real = "SELECT id_usuario FROM usuario WHERE id_usuario = $id_cuidador LIMIT 1";
        $res_check_real = $conexion->query($check_cuidador_real);
        
        if ($res_check_real && $res_check_real->num_rows > 0) {
    ...
    // --- PASO B: GUARDAR LOS MEDICAMENTOS CON HORARIO REAL ---
